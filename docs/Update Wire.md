...
> **Note:** the old wire's timeline group is deleted with its contents — but only when it is still the group Route Wire created (its name matches `Wire <name>` / `Cable <name>`). A renamed or user-made group is left alone and only the wire assembly itself is deleted.
