...
PowerTools Preferences control (`PT-preferences`). It is a launcher: on
...
`commands/__init__.py`), the `PT-preferences` anchor exists by the time this
...
`PT-scriptsmanager` (launches built-in `ScriptsManagerCommand`)

### Execution flow

1. The add-in registers the command definition and inserts a control in the QAT
   File dropdown, positioned before `PT-preferences`.
...
C4Component
    ...
    Rel(addin, cmd, "Calls start() – inserts control before PT-preferences")
