...
C4Context
  ...
  System_Ext(ospicker, "OS Color Picker", "macOS: AppleScript 'choose color' via /usr/bin/osascript; elsewhere: bundled-Python tkinter.colorchooser")
...
C4Component
  ...
  Container_Boundary(cmd, "commands/changecyclecolor") {
    ...
    Component(install, "fusion_install.py", "Fusion path resolver", "find_river_rubicon_xml walks up from adsk.__file__ to locate RiverRubicon.xml inside the active webdeploy install")
...
sequenceDiagram
    ...
    alt macOS
        ...
        Picker->>Picker: bundled python _color_picker_subprocess.py (tkinter.colorchooser → hex on stdout)
    end
    Picker-->>Entry: (r, g, b) tuple, or None on cancel
...
- **`fusion_install.py`** — `find_river_rubicon_xml` discovers `RiverRubicon.xml` dynamically by walking up from `adsk.__file__` (falling back to `sys.executable`), so the path tracks Fusion `webdeploy` hash changes automatically.
