...
### Why do the hand-off buttons read the preferences store?
**Assembly Builder…** and **Global Parameters…** execute another PowerTools command by id. A command switched off in Preferences (or whose whole group is off, or a beta command with beta mode off) is never started, so `commandDefinitions.itemById` returns `None` and `_execute_command` could only report *"Command 'X' is not available"* — after the palette had already hidden itself for the hand-off. The buttons are now hidden instead of left on screen leading nowhere.

The check goes through `settings_store.is_command_available(module)`, which looks the module up in `command_registry` and applies `settings_store.is_enabled(group, cmd, prefs)` — the *same* function `commands/__init__.py` now uses for its start-up gate, so the palette and the loader can never disagree about what is running. (That rule previously lived as a private `_should_start` in the loader; duplicating it here would have been the obvious way for the two to drift.) An unregistered module answers `False`, so `test_handoff_targets_are_actually_registered` pins both keys — a rename would otherwise silently hide the buttons for everyone, exactly as the `assemblyintent` → `assemblypalette` rename already caught out `settings_store.RENAMED_COMMANDS`.

Availability rides in the palette state as `handoffs` (baked into `init.js`, re-pushed as `setHandoffs` on reopen). The page treats an *absent* answer as "show it" — a stale cached `init.js`, which Windows' embedded browser is known to serve, must never blank out working buttons. `_launch_handoff` re-checks server-side before hiding the palette, covering a page loaded before the setting changed.

Hiding is per row *and* its preceding `.divider`, or a disabled command would leave a rule with nothing under it. `.action-row` sets `display: flex`, and a class selector beats the UA's `[hidden] { display: none }`, so `style.css` says so explicitly.
