...
Two details of that placement were paid for. `INDEX_DY` is `HALO_R + 2.5`, not `NODE_R + 2`: anchored at the dot's own radius the last character landed inside the ring a milestone and a release wear, on exactly the two marks whose numbers matter most. And the label rises *left*, ending at its dot (`INDEX_ANCHOR` of `end` with a positive `INDEX_ANGLE`), so the number reads into the event rather than away from it; the cost is that a save near midnight has its label clipped by the left edge of the plot, and rising right only moves that problem to the right edge. The two constants flip together.

`indexLabelColor` gives a label its mark's own colour, following `dotNode` and `changeNode`: accent for a release, the author's rail colour for a change, `C.secondary` for an ordinary save or a milestone — whose dot is that colour too, the ring rather than the fill saying it was marked. `rowNode` computes that rail colour once and hands it to both the ring and the label, so the two cannot drift.
