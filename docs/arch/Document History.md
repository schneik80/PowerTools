...
`indexLabelText` is what a label prints, as against `indexLabel` which is what the Python stamped. The major is dropped while it is still zero: before the first release it cannot vary, so it spends a third of every label — and, the labels being angled, a third of the vertical room they need — saying nothing. A pre-release `1.0` and a released `1.0.0` differ in length rather than in value, which is legible because the release is also the only label drawn in the accent. The hover card prints the same abbreviated form, so there is one label per event rather than two encodings of it, and prints it unconditionally: gating it on `showIndex` made sense while the toggle was the only way to see a number, but once hovering a track revealed them it meant the one place a reader was already pointing at an event was the place that would not name it.
