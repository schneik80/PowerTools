...
`RAIL_FRAC` puts a track's rail at 80% of its own height rather than the middle, and `ROW_PAD_Y` is the lead-in above the first one. Both exist for the angled index labels: the rail is low so the labels have somewhere to rise, and the lead-in is the top track's share of that room — at 45 degrees the longest label rises about 25px, which clipped against the top of the plot at the old padding. The avatar disc is offset by the same fraction with a `translateY`, not padding, so the gutter meets the rail it labels without any cell changing height and pushing the next one down.
