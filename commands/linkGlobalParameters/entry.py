...
def _refresh_param_doc_map(project) -> None:
    ...
    global _param_doc_map, _param_doc_entries
    with ptutil.perf_timer("list_param_docs (forced Hub refresh)", "LGP.docs_resolve"):
        _param_doc_map = cache.list_param_docs(project, CMD_NAME)
    _param_doc_entries = [
        {"name": name, "id": getattr(data_file, "id", "")}
        for name, data_file in _param_doc_map.items()
...
def _load_preview(
    ...
    if cached_params is not None:
        with ptutil.perf_timer("load_preview (sidecar)", "LGP._load_preview"):
            _populate_preview_table(cached_params, inputs, table)
        return

    # ── Fallback: open the document via the Fusion API ────────────────────────
    try:
        with ptutil.perf_timer("documents.open", "LGP._load_preview"):
...
def _derive_into_active(data_file, active_doc: adsk.core.Document) -> None:
    ...
    with ptutil.perf_timer("documents.open", "LGP._derive_into_active"):
        params_doc = app.documents.open(data_file, False)
    try:
        params_design = adsk.fusion.Design.cast(
            params_doc.products.itemByProductType("DesignProductType")
        ...
        with ptutil.perf_timer("derive_features.add", "LGP._derive_into_active"):
            derive_features.add(derive_input)
        timeline.markerPosition = timeline.count

        with ptutil.perf_timer("mark derived favorites", "LGP._derive_into_active"):
        ...
        ptutil.log(f"{CMD_NAME}: parameters derived from '{data_file.name}'")
    finally:
        with ptutil.perf_timer("close + reactivate", "LGP._derive_into_active"):
            params_doc.close(False)
...
def command_created(args: adsk.core.CommandCreatedEventArgs):
    ...
    with ptutil.perf_timer("list_param_docs (Hub scan)", "LGP.command_created"):
        _param_doc_map = cache.list_param_docs(project, CMD_NAME)
    _param_doc_entries = [
        {"name": name, "id": getattr(data_file, "id", "")}
        for name, data_file in _param_doc_map.items()
