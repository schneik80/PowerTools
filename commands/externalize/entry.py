...
def command_input_changed(args: adsk.core.InputChangedEventArgs):
    ptutil.log(f"{CMD_NAME} Input Changed Event")
    ...
    try:
        ...
        if create_log:
            try:
                ...
                ptutil.log(f"Failed to initialize log file: {log_e}")
                create_log = False

        log_writer = _LogWriter(log_path_val if create_log else None)

        if create_log and open_log_view:
            _, msg = ptutil.open_live_log_viewer(log_path_val)
            log_writer(msg)
...
class _RunnerHandler(adsk.core.CustomEventHandler):
    ...
    def notify(self, args):
        ...
        if run is None:
            ptutil.log(f"{CMD_NAME}: handler fired with no pending run — ignoring")
            return
...
def _default_log_path() -> str:
    ...
    return os.path.join(ptutil.default_log_directory(), _propose_default_log_filename())


class _LogWriter:
    """Append-on-call writer. When path is None, lines go only to ptutil.log
    ...
    def __call__(self, line: str):
        ...
        try:
            ...
            ptutil.log(f"Failed to write log entry: {e}")
