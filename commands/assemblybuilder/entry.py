...
def command_execute(args: adsk.core.CommandEventArgs):
    ...
    if just_created:
        ...
        ptutil.add_handler(palette.closed, palette_closed)
        ptutil.add_handler(palette.navigatingURL, palette_navigating)
        ptutil.add_handler(palette.incomingFromHTML, palette_incoming)
        # Keep the palette's save-required gate in sync when the user saves.
        ptutil.add_handler(app.documentSaved, document_saved)
        ptutil.log(
...
def _gather_palette_state() -> dict:
    ...
    if _active_project_ref is None:
        ptutil.log(f"{CMD_NAME}: no active project — cannot list parameter docs.")
    else:
        try:
...
def document_saved(args: adsk.core.DocumentEventArgs):
    ...
    ptutil.log(f"{CMD_NAME}: Document saved — notifying palette ({state}).")
    palette.sendInfoToHTML("setSaveState", state)


def palette_closed(args: adsk.core.UserInterfaceGeneralEventArgs):
    ptutil.log(f"{CMD_NAME}: Palette was closed.")


def palette_navigating(args: adsk.core.NavigationEventArgs):
    url = args.navigationURL
    ptutil.log(f"{CMD_NAME}: Palette navigating to {url}")
...
def _derive_param_links(
    ...
    for pname, pdf in root_items:
        try:
            ...
            derived += 1
            ptutil.log(f"  Derived '{pname}' into root")
        except Exception as e:
    ...
    try:
        ...
        ptutil.log(f"{CMD_NAME}: updateAllReferences failed: {e}")
    try:
        root_doc.save("Updated with Assembly Builder")
    except Exception as e:
        ptutil.log(f"{CMD_NAME}: final root save failed: {e}")
...
def create_assembly_from_graph(graph_data: dict) -> str:
    ...
    def create_children(parent_comp: adsk.fusion.Component, parent_node_id: int):
        ...
        for child_id in get_child_ids(nodes, parent_node_id):
            ...
            ptutil.log(f"  Creating external component: {child_name} ({child_type})")

            occ = parent_comp.occurrences.addNewExternalComponent(
                child_name, folder, transform
            )
            if not occ:
                ptutil.log(f"  Failed to create: {child_name}")
                continue
            ...
            if intent is not None:
                try:
                    ...
                    ptutil.log(f"  Set {child_type} intent on {child_name}")
                except Exception as e:
                    ptutil.log(f"  Failed to set intent on {child_name}: {e}")

            # Recurse for assemblies/hybrids
            if child_type in ("assembly", "hybrid"):
                create_children(child_comp, child_id)

    ptutil.log(f"{CMD_NAME}: Creating assembly hierarchy...")
    if has_shared:
        ptutil.log(f"{CMD_NAME}: Shared nodes detected: {shared_node_ids}")
    ...
    if need_flush:
        ...
        try:
            ...
            ptutil.log(f"{CMD_NAME}: Flush save failed: {e}")
        adsk.doEvents()
    ...
    if deferred_inserts:
        for parent_node_id, parent_comp, child_id in deferred_inserts:
            ...
            try:
                ...
                ptutil.log(f"  Shared insert error for {child_name}: {e}")

    # Pass 3: derive global parameters into the components that link them.
    derived_count = 0
    param_errors: list[str] = []
    if param_links:
        ptutil.log(f"{CMD_NAME}: Deriving global parameters into linked components...")
