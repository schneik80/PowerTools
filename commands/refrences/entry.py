    ...
    try:
        ...
        def _collect_roots(data_file, visited_ids: set, root_ids: set, root_items: list, depth: int = 0):
            ...
            try:
                ...
                ptutil.log(f"[Roots]{indent} SKIP — could not read id/name")
                return

            ptutil.log(f"[Roots]{indent} Visiting: '{fname}'  id={fid}")

            if fid in visited_ids:
                ptutil.log(f"[Roots]{indent} SKIP — already visited: '{fname}'")
                return
            visited_ids.add(fid)
            ...
            for p in (parents or []):
                ...
                if _is_drawing(p):
                    ...
                    ptutil.log(f"[Roots]{indent} KEEP parent: '{pname}'")
        ...
        def fetch_thumbnail(data_file) -> str:
            ...
            if component:
                try:
                    ...
                    if data_obj and _save_data_object(data_obj, dest):
                        ...
                        ptutil.log(f"[Thumbnail] OK (component): {data_file.name}")
                        return dest
                    ptutil.log(
            ...
            try:
                ...
                if future is None:
                    ptutil.log(f"[Thumbnail] No future for: {data_file.name}")
                ...
                if data_obj and _save_data_object(data_obj, dest):
                    _thumb_cache[file_id] = dest
                    _thumb_paths.append(dest)
                    ptutil.log(f"[Thumbnail] OK (future): {data_file.name}")
                    return dest
