...
def _write_pending_cache(
    ...
    ptutil.log(f"{CMD_NAME}: pending cache written → {path}")
...
def _read_pending_cache(doc: adsk.core.Document) -> dict | None:
    ...
    try:
        ...
        ptutil.log(f"{CMD_NAME}: failed to read pending cache — ignoring")
        return None
...
def _clear_pending_cache(doc: adsk.core.Document) -> None:
    ...
    if path and os.path.exists(path):
        try:
            os.remove(path)
            ptutil.log(f"{CMD_NAME}: pending cache deleted → {path}")
        except Exception:
            ptutil.log(f"{CMD_NAME}: failed to delete pending cache — ignoring")
...
def _ensure_param_doc_map_loaded(project) -> None:
    ...
    if _param_doc_map:
        return
    with ptutil.perf_timer("list_param_docs (lazy Hub scan)", "GP.docs_resolve"):
...
def _load_parameters_from_doc(
    ...
    try:
        ...
        with ptutil.perf_timer(f"build table rows (n={n})", "GP._load_params_from_doc"):
            for i in range(n):
...
def _update_parameters_document(
    ...
    """
    with ptutil.perf_timer("documents.open", "GP._update_params_doc"):
    ...
    try:
        ...
        raise
    with ptutil.perf_timer("documents.close", "GP._update_params_doc"):
        params_doc.close(False)
    cache.safe_activate(active_doc, CMD_NAME)
...
def _create_parameters_document(
    ...
    with ptutil.perf_timer("find_global_params_folder", "GP._create_params_doc"):
    ...
    with ptutil.perf_timer("documents.add (new design doc)", "GP._create_params_doc"):
    ...
    try:
        ...
        with ptutil.perf_timer("document.saveAs (Hub upload)", "GP._create_params_doc"):
            params_doc.saveAs(
    ...
    cache.safe_activate(active_doc, CMD_NAME)
    ptutil.log(f"{CMD_NAME}: parameters document saved → {doc_name}")
...
def _apply_parameters(inputs: adsk.core.CommandInputs) -> str | None:
    ...
    if not parameters:
        ptutil.log(f"{CMD_NAME}: no parameters collected — skipping document creation.")
...
def command_created(args: adsk.core.CommandCreatedEventArgs):
    ...
    with ptutil.perf_timer("list_param_docs (Hub scan)", "GP.command_created"):
        _param_doc_map = cache.list_param_docs(project, CMD_NAME)
    _param_doc_names = list(_param_doc_map.keys())
    ...
    with ptutil.perf_timer("read_pending_cache", "GP.command_created"):
...
def command_input_changed(args: adsk.core.InputChangedEventArgs):
    ...
    inputs = args.inputs
    if config.DEBUG:
        ptutil.log(f"{CMD_NAME} Input Changed: {changed.id}")
    ...
    if changed.id == MODE_INPUT_ID:
        ...
        if new_selection == CREATE_NEW_LABEL:
            ...
            if data_file is not None:
                with ptutil.perf_timer("load_parameters_from_doc", "GP.input_changed"):
