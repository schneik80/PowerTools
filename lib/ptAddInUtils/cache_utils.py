...
# add-in root is three levels up: lib/ptAddInUtils/ → lib/ → add-in root
...
def read_global_params_folder_cache(project, cmd_name: str) -> dict | None:
    ...
    try:
        ...
        ptutil.log(f"{cmd_name}: failed to read folder cache — ignoring")
        return None
...
def find_global_params_folder(project, cmd_name: str):
    ...
    with ptutil.perf_timer("folder cache fast-path", f"{cmd_name}._find_param_folder"):
...
def read_param_docs_cache(project, cmd_name: str) -> list[dict]:
    ...
    try:
        ...
        ptutil.log(f"{cmd_name}: failed to read docs cache — ignoring")
        return []
...
def write_param_docs_cache(project, doc_map: dict, cmd_name: str) -> None:
    ...
    try:
        ...
        ptutil.log(f"{cmd_name}: failed to write docs cache — ignoring")
...
def upsert_param_docs_cache_entry(
    ...
    try:
        ...
        ptutil.log(f"{cmd_name}: failed to upsert docs cache entry — ignoring")


def list_param_docs(project, cmd_name: str) -> dict:
    """Return {doc_name: DataFile} for all docs in the '_Global Parameters' folder."""
    with ptutil.perf_timer("find_global_params_folder", f"{cmd_name}._list_param_docs"):
...
def write_param_set_sidecar(data_file, parameters: list, cmd_name: str) -> None:
    ...
    *parameters* is the list of dicts produced by globalParameters._collect_rows()
    ...
    try:
        ...
        with open(path, "w", encoding="utf-8") as fh:
            json.dump(payload, fh, indent=2)
        ptutil.log(f"{cmd_name}: param set sidecar written → {path}")
    except Exception:
        ptutil.log(f"{cmd_name}: failed to write param set sidecar — ignoring")
...
def get_active_project(cmd_name: str):
    ...
    try:
        ...
        ptutil.log(f"{cmd_name}: could not retrieve active project — ignoring")
        return None
...
def safe_activate(doc: adsk.core.Document, cmd_name: str) -> None:
    ...
    try:
        ...
        ptutil.log(f"{cmd_name}: could not re-activate original document — ignoring")
