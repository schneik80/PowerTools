...
| Diagnostics missing from `cache/powertools-debug.log` after a crash | Logged through `assemblypalette._diag`, which only reaches the Text Commands window | Use `ptutil.log` for anything that must survive the session |
...
| CER stack aborts in `Ns::_AutoSaveTask` → `SegmentSaver::save` → `doSave` → `std::terminate`, no Python frame | A main-thread read of the document model (`documentReferences`, `dataFile`) from an application event raced Fusion's background saver. An *idle* `_AutoSaveTask` in `condition_variable::wait_for` with no abort frame is the benign shape | Never read the model from `documentSaved`; defer refreshes through `fireCustomEvent` + `threading.Timer` (`docs/arch/Assembly Palette.md`, "Attempted and parked") |
...
| A units comparison disagrees with what Document Settings shows | Compared `UnitsManager.defaultLengthUnits` strings, which the user's abbreviation/symbol preferences reshape (`inch` / `in` / `"`) | Compare `FusionUnitsManager.distanceDisplayUnits` + `massDisplayUnits` enums against `app.preferences.defaultUnitsPreferences.itemByName("Design")` (`commands/matchunits/`) |
| Toolpath feeds, speeds or dimensions are in the wrong unit | The **Manufacture** product keeps its own active unit system, separate from the design's, and Fusion never says so | Match Units' Manufacture check (`prompt_on_manufacture`). Read/write only via `UnitSystems.List` / `UnitSystems.Activate <id>`; there is no CAM units API (`commands/matchunits/mfg.py`) |
| A state badge or enabled-state goes stale, or reports the wrong verdict, after a workspace switch | `Design.cast(app.activeProduct)` returns `None` outside Design, so the refresh took its "nothing to show" path | Resolve the design with `doc.products.itemByProductType("DesignProductType")`, and repaint from `workspaceActivated` as well as `documentActivated` |
